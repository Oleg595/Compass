...
public class CompareValueService {
    ...
    private void processAnswer(
        DMatrixRMaj matrix, int x, int y, int ans, CriteriaEntity criteria, String valueX, String valueY) {
        AlternativePair pair;
        switch (ans) {
            case 1:
                matrix.set(x, y, 1.);
                matrix.set(y, x, -1.);
                pair = AlternativeUtils.calculateAlternativePair(
                    dataContext.getCriterias(), List.of(criteria), List.of(valueX), List.of(valueY));
            ...
            case 2:
                ...
                matrix.set(x, y, -1.);
                pair = AlternativeUtils.calculateAlternativePair(
                    dataContext.getCriterias(), List.of(criteria), List.of(valueY), List.of(valueX));
                dataContext.addP(pair);
                return;
            case 3:
                matrix.set(x, y, .5);
